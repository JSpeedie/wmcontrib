...
void print_help(void) {
	fprintf(stderr, "usage: wam -[xy] <new_location> -w <wid> [OPTIONS]\n");
}
...
int main(int argc, char **argv) {
	...
	if (use_x == 0 && use_y == 0) {
		fprintf(stderr, "wam: Neither an x nor y were given.\n");
		print_help();
		exit(ERR_NEITHER_RATIO_GIVEN);
	}
	if (window_given == 0) {
		fprintf(stderr, "wam: Window ID was not given.\n");
		print_help();
		exit(ERR_WINDOW_NOT_GIVEN);
	}
	if (window_exists(win_id) != 0) {
		fprintf(stderr, "wam: Could not find window of given id.\n");
		exit(ERR_WIN_NOT_FOUND);
	}

	Display *dpy;
	if (!(dpy = XOpenDisplay(0))) {
		fprintf(stderr, "wam: Couldn't open X display\n");
